CREATE OR REPLACE VIEW kmdata.vw_Artwork AS 
 SELECT w.id, w.resource_id, w.user_id, w.artist, w.city, w.country, w.curator, 
        ...
        artist, city, country, curator, dimensions, exhibit_title,
	          juried_ind, medium, percent_authorship, exhibition_type_id, sponsor, state, title, url, venue,
	          creation_dmy_single_date_id, 
	          exhibit_dmy_range_date_id, rd.start_month, rd.start_year, rd.end_month, rd.end_year,
	           work_type_id, a.work_type_name, completed, ongoing, other_artist, solo, f.name as solo_name, sub_work_type_id, e.name as work_type_name,
           sd.year, sd.month, sd.day, b.narrative_text, c.is_public
   FROM kmdata.works w
