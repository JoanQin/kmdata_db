CREATE OR REPLACE FUNCTION kmdata.import_published_works (
) RETURNS BIGINT AS $$
DECLARE
   ...
   FOR currWork IN worksQuery LOOP
      ...
      IF v_intUserCount = 1 THEN
         ...
         IF v_intMatchCount <= 0 THEN
            ...
            SELECT b.id INTO v_intKMDataWorkTypeID
            FROM osupro.publication_type_ref a
            INNER JOIN kmdata.work_types b ON a.publication_type_descr = b.work_type_name
            WHERE a.publication_type_id = currWork.publication_type_id;
   
            -- read from the sequence
            v_intWorkID := nextval('kmdata.works_id_seq'); -- v_intPublicationID := nextval('kmdata.publications_id_seq');
   
            -- insert the article using the publication ID
            INSERT INTO kmdata.works
               (id, work_type_id, journal_article_type_id, publication_year, publication_month, 
                publication_day, author, editor, reviewer, article_title, journal_title, review_type_id, edition, volume, issue, 
                technical_report_number, submitted_to, submission_date, city, state, country, series, sponsor, beginning_page, 
                ending_page, url, percent_authorship, description, author_date_citation, bibliography_citation, research_report_citation, 
                research_report_ind, impact_factor, issn, publisher, publication_location, collection, site_last_viewed_date, publication_media_type_id, 
                status_id, abstract_type_id, peer_reviewed_ind, submission_year, submission_month, submission_day, presentation_role_id, 
                presentation_location_descr, presentation_location_type_id, audience_name, event_title, invited_talk, publication_title, 
                old_compiler, degree, acceptance_rate, 
                presentation_dmy_range_date_id,
                --presentation_start_year, presentation_start_month, presentation_start_day, 
                --presentation_end_year, presentation_end_month, presentation_end_day, 
                role_id)
            VALUES
               (v_intWorkID, v_intKMDataWorkTypeID, currWork.journal_article_type_id, currWork.publication_year, currWork.publication_month, 
                ...
                currWork.old_compiler, currWork.degree, currWork.acceptance_rate, 
                kmdata.add_dmy_range_date(currWork.presentation_start_day, currWork.presentation_start_month, currWork.presentation_start_year, 
                   currWork.presentation_end_day, currWork.presentation_end_month, currWork.presentation_end_year), 
                currWork.role_id);
   
         ELSE
   
            -- get the publication id
            SELECT p.id INTO v_intWorkID
            FROM kmdata.works p
            WHERE p.article_title = currWork.article_title
            AND (p.journal_title = currWork.journal_title OR p.journal_title IS NULL)
            AND (p.publication_type_id = currWork.publication_type_id OR p.publication_type_id IS NULL)
         ...
         BEGIN
            INSERT INTO kmdata.work_authors
               (work_id, user_id)
            VALUES
               (v_intWorkID, v_intUserID);
         EXCEPTION
            WHEN OTHERS THEN
               INSERT INTO kmdata.import_errors (error_number, message_varchar) 
               VALUES (SQLSTATE, 'INSERT into kmdata.work_authors failed with work_id == ' || 
                       CAST(v_intWorkID AS CHAR) || ' and user_id == ' || CAST(v_intUserID AS CHAR) || '. Not inserted.');
         END;
